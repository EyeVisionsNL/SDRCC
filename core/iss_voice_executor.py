...
def _remove_validated_iq(*, capture: dict[str, Any] | None, config: dict[str, Any]) -> dict[str, Any]:
    """Remove raw ISS IQ only after the mission has fully succeeded.

    The capture path is generated by WidebandIQRecorder under the ISS recordings
    tree. Failed/incomplete missions never call this helper.
    """
    storage = config.get("storage") or {}
    if bool(storage.get("keep_raw_iq", False)):
        return {"removed": False, "reason": "keep_raw_iq"}
    iq_value = (capture or {}).get("iq_path")
    if not iq_value:
        return {"removed": False, "reason": "no_iq_path"}
    iq_path = Path(str(iq_value)).expanduser().resolve()
    recordings_root = (Path(__file__).resolve().parent.parent / "data" / "recordings" / "iss_voice").resolve()
    try:
        iq_path.relative_to(recordings_root)
    except ValueError as exc:
        raise RuntimeError("Refusing to remove ISS IQ outside recordings root") from exc
    if iq_path.name != str(config.get("iq_filename") or "recording.iq"):
        raise RuntimeError("Refusing to remove unexpected ISS IQ filename")
    if not iq_path.exists():
        return {"removed": False, "reason": "already_absent", "path": str(iq_path)}
    iq_path.unlink()
    return {"removed": True, "reason": "retention_policy", "path": str(iq_path)}
...
def execute_pass(*, target: dict[str, Any], service_state: ServiceState,
    ...
    iq_retention = {"removed": False, "reason": "mission_failed"}
    if failure is None:
        try:
            iq_retention = _remove_validated_iq(capture=capture, config=cfg)
            if iq_retention.get("removed"):
                event_bus.publish_mission(
                    "INFO", "ISS Voice raw IQ removed", str(iq_retention.get("path") or "recording.iq"),
                    data={"plugin_id": "iss_voice", "mission_id": mission_id, "retention": "wav_only"},
                )
        except Exception as exc:
            # Retention cleanup must never turn a successfully recorded mission into a failure.
            iq_retention = {"removed": False, "reason": "cleanup_error", "error": str(exc)}
    ...
    iss_voice_runtime.finish(
        ...
        iq_path=None if iq_retention.get("removed") else (capture or {}).get("iq_path"),
        iq_bytes=0 if iq_retention.get("removed") else (capture or {}).get("actual_bytes"),
        wav_path=(audio or {}).get("wav_path"),
    )
    event_bus.publish_mission(
