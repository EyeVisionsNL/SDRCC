...
def validate_configuration(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not isinstance(backend, dict):
        ...
        try:
            ...
            scan_interval_ms = 0
        if scan_interval_ms < 100 or scan_interval_ms > 500 or scan_interval_ms % 50:
            errors.append("backend.scan_interval_ms must be between 100 and 500 ms in 50 ms steps")
...
def normalize_receiver_settings(
    ...
    *,
    ...
    try:
        ...
        raise ValueError("Ongeldige Traffic Voice-scansnelheid") from error
    if scan_interval_ms < 100 or scan_interval_ms > 500 or scan_interval_ms % 50:
        raise ValueError("Scansnelheid moet 100-500 ms per kanaal zijn in stappen van 50 ms")
...
def export_channel_list_xlsx(payload: dict[str, Any] | None = None) -> bytes:
    ...
    notes["A1"] = "FlexGround SDR Traffic Voice Channel List"; notes["A1"].font = Font(bold=True)
...
def import_channel_list_xlsx(data: bytes, *, source_name: str = "channels.xlsx") -> dict[str, Any]:
    ...
    if header[:len(CHANNEL_LIST_HEADERS)] != list(CHANNEL_LIST_HEADERS):
        raise ValueError("Channels sheet headers do not match the FlexGround SDR export format.")
...
def render_rtlsdr_airband_config() -> str:
    ...
    return "\n".join((
        "# Generated by FlexGround SDR v0.56.0f; do not edit runtime output.",
