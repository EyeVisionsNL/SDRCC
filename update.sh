...
mkdir -p "$BACKUP/config"; cp -a "$ROOT/config/." "$BACKUP/config/"
git -C "$ROOT" pull --ff-only
"$ROOT/venv/bin/pip" install -r "$ROOT/requirements.txt"
"$ROOT/scripts/install/provision_external.sh" --check || echo "WARN: external provisioning differs from the v1.0 reference stack"
