...
function renderMissionDetail(payload) {
    ...
    const galleryImageKey = item => String(item?.relative_path || item?.url || item?.filename || "");
    ...
    function selectGalleryImage(item, button = null) {
        if (!item?.url) return;
        if (missionId) selectedGalleryImageByMission.set(missionId, galleryImageKey(item));
    ...
    if (imageFiles.length) {
        ...
        imageFiles.forEach((item, index) => {
            ...
            button.dataset.imageKey = galleryImageKey(item);
            thumbnails.appendChild(button);
        });

        const wantedKey = selectedGalleryImageByMission.get(missionId);
        const selectedIndex = Math.max(0, imageFiles.findIndex(item => galleryImageKey(item) === wantedKey));
        const selectedItem = imageFiles[selectedIndex] || imageFiles[0];
        const selectedButton = thumbnails.querySelector(`[data-image-key="${CSS.escape(galleryImageKey(selectedItem))}"]`)
            || thumbnails.querySelector(".history-gallery-thumb");
        selectGalleryImage(selectedItem, selectedButton);
