...
async function updateLiveCapture() {
    try {
        ...
        // Do not overwrite a manually selected item in the Images gallery.
        const imagesViewer = document.getElementById("capture-image-images");
        const galleryHasManualSelection = Boolean(
            imagesViewer && imagesViewer.src.includes("?selected=")
        );
        if (!galleryHasManualSelection) {
