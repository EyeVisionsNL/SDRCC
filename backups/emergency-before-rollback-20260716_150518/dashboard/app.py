...
def _mission_output_inventory(mission):
    """Build one canonical inventory for every Mission History consumer.

    Image discovery deliberately reuses _mission_capture_files(), the same
    provider used by Mission Control and the Images tab. This prevents the
    history detail page from reporting zero image files while the capture API
    can already see the decoded products.
    """
    output_value = str(mission.get("output_path") or "").strip()
    inventory = {
        "available": False,
    ...
    root, image_candidates = _mission_capture_files(mission)
    if root is None:
        return inventory
    ...
    image_set = {path.resolve() for path in image_candidates}
    for path in files[:5000]:
        ...
        if resolved in image_set or suffix in MISSION_IMAGE_EXTENSIONS:
            bucket = "images"
    ...
    # Keep the same useful-product ordering as Mission Control / Images.
    for path in image_candidates[:500]:
        ...
        item = capture_to_dict(path, mission=mission, root=root)
        item.update({
            "directory": str(relative.parent).replace(chr(92), "/"),
            "bytes": stat.st_size,
        })
        inventory["image_files"].append(item)

    if inventory["image_files"]:
        first = inventory["image_files"][0]
        inventory["preview"] = {
