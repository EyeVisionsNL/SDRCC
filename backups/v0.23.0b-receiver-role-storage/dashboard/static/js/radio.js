(() => {
    ...
    if (receiverRolesForm) {
        ...
        receiverRolesForm.addEventListener("submit", (event) => {
            event.preventDefault();
            ...
            if (result) {
                result.textContent = `UI-controle geslaagd: SDR1 → ${sdr1.toUpperCase()}, SDR2 → ${sdr2.toUpperCase()}. Nog niet opgeslagen.`;
            }
        });
    }
