# FlexGround SDR 0.56.0q-r2 — flexibele ontvangers

Dit pakket vervangt het eerdere r1-pakket. Eén install.sh ondersteunt een nieuwe Ubuntu-installatie (pc, mini-pc of Raspberry Pi met Ubuntu) en een bestaande 0.56.0p-installatie. Het bevat Python-broncode; externe radioprogramma's worden op de doelmachine geïnstalleerd/gebouwd via de bestaande provisioning.
...
```bash
cd ~/Downloads
tar -xzf flexground-sdr-v0.56.0q-r2-flexible-receivers.tar.gz
cd flexground-sdr-v0.56.0q-r2-flexible-receivers
./install.sh
```
...
Receiver Inventory krijgt de fysieke status en een bindingsformulier binnen het bestaande tabblad. Geen extra navigatietab, tweede registry of tweede servicecontroller. Installatiedetectie en core/rtl.py gebruiken dezelfde fysieke detector. Bewuste presentatieherhaling blijft bestaan in Runtime en Inventory. Compatibility-aliassen blijven behouden.

## Validatie en beperkingen

30 gedragstests gebruiken de echte Registry/Manager en tijdelijke bestanden. USB-descriptors en serviceacties zijn gesimuleerd. Getest: lege slots, behoud bij ontbreken/fouten, eerste binding, één/twee vervangers, onduidelijke aantallen, dubbele/lege serienummers, handmatige binding, reserverings- en activatieblokkade, executor-opruiming, behoud van herstelintentie na restart, stop/sync/start, expliciete Stop, AIS-volgorde, mislukte serviceherstart, mislukte externe sync, atomische schrijffout en idempotent herstel. Ook locatieconfiguratie zonder dongles en het aansluiten van schone serviceconfiguraties op de bestaande helper zijn getest.
...
De update toont een backupmap onder ~/SDRCC/.rollback/v0.56.0q-r2-DATUM-TIJD. Daarin staan de vorige bronbestanden, een configuratiebackup en rollback_code.py. Het uitgeprinte rollbackcommando herstelt broncode en behoudt de huidige lokale instellingen en receiverbindingen. Het weigert tijdens reserveringen of onafgehandeld hardwareherstel. Herstel receivers.yaml niet los nadat dongles zijn vervangen: de externe serviceconfiguratie moet dezelfde binding houden.
