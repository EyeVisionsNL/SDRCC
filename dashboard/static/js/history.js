...
function missionCard(mission) {
    ...
    metrics.append(
        ...
        metric(window.SDRCC_UI_TEXT.t("images"), number(mission.image_count)),
...
function qualityItem(label, value, displayValue = null) {
    ...
    content.textContent = displayValue === null ? (value ? "OK" : "Not confirmed") : text(displayValue);
...
function fileItem(icon, label, info = {}) {
    ...
    meta.textContent = info.available
        ...
        : window.SDRCC_UI_TEXT.t("not_available");
...
function renderMissionDetail(payload) {
    ...
    deleteButton.setAttribute("aria-label", window.SDRCC_UI_TEXT.tf("delete_mission_aria", {mission: text(mission.mission_id)}));
    deleteButton.textContent = window.SDRCC_UI_TEXT.t("delete");
    ...
    qualityGrid.append(
        ...
        qualityItem(window.SDRCC_UI_TEXT.t("images"), Number(quality.images || 0) > 0, number(quality.images, "0")),
    ...
    if (imageFiles.length) {
        ...
        empty.textContent = window.SDRCC_UI_TEXT.t("no_mission_image");
    ...
    if (events.length) {
        ...
        empty.textContent = "No saved Event Bus events for this mission.";
    ...
    technical.append(
        ...
        detailItem(window.SDRCC_UI_TEXT.t("diagnostics"), diagnostics.available ? diagnostics.directory : window.SDRCC_UI_TEXT.t("not_available"), {wide: true, code: true}),
...
async function deleteMission(mission, button) {
    ...
    const originalLabel = button?.textContent || window.SDRCC_UI_TEXT.t("delete");
    if (button) {
        button.disabled = true;
        button.textContent = window.SDRCC_UI_TEXT.t("deleting");
    }
    ...
    try {
        ...
        if (!response.ok || payload.ok === false) {
            throw new Error(payload.error || window.SDRCC_UI_TEXT.t("delete_failed"));
        }

        selectedMissionId = null;
        renderEmptyDetail(window.SDRCC_UI_TEXT.t("deleted_refreshing"));
        await refreshMissionHistory();
    } catch (error) {
        window.alert(window.SDRCC_UI_TEXT.tf("delete_error", {error: error.message}));
        if (button) {
...
export async function refreshMissionHistory() {
    ...
    try {
        ...
        renderEmptyDetail(window.SDRCC_UI_TEXT.t("mission_detail_unavailable"));
    }
}
